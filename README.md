...
- API local: `http://127.0.0.1:8000/health`
...
## 9) Sửa lỗi Vercel "No python entrypoint found"

- Cách đúng: deploy frontend riêng với **Root Directory = `webapp`**.
- Hoặc dùng `vercel.json` (đã thêm trong repo) để ép Vercel static build từ `webapp/`.
- Nếu Vercel vẫn lỗi, xóa project cũ và import lại từ đầu để reset build settings.


## 10) Không dùng Docker

- Bạn có thể bỏ qua `Dockerfile` và `docker-compose.yml`.
- Với Railway, service API chạy bằng `railway.json` + lệnh `uvicorn`, không bắt buộc Docker.
